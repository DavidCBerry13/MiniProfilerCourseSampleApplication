...
{
    internal class SqlSecurityPriceRepository : ISecurityPriceRepository
    {

        internal SqlSecurityPriceRepository(String connectionString)
        {
	...
	INNER JOIN SecurityTypes st
        ...
        public List<SecurityPrice> LoadSecurityPrices(TradeDate date)
        {
            ...
            {
                con.Open();
                String sql = $"{SQL} WHERE p.TradeDate = @TradeDate";
                using (SqlCommand cmd = new SqlCommand(sql, con))
                {
                    cmd.Parameters.AddWithValue("@TradeDate", date.Date);
        ...
        public List<SecurityPrice> LoadSecurityPrices(string symbol)
        {
            ...
            {
                con.Open();
                String sql = $"{SQL} WHERE p.Ticker = @Symbol";
                using (SqlCommand cmd = new SqlCommand(sql, con))
                {
        ...
        public SecurityPrice DecodeRow(SqlDataReader reader)
        {
